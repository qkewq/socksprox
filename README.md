...
The SOCKS5 standard defines methods for authentication identified by their method specifier.  The RFC defines the methods `0x00 No Authentication`, `0x01 GSSAPI`, and `0x02 Username/passsword`.  It states that methods `0x03-0x7F` are IANA assigned and `0x80-0xFE` are reserved for private use.  While IANA has assigned methods `0x03-0x09` there are no official standards tied to these methods.
...
|GSSAPI|gssapi|[RFC 1961](https://datatracker.ietf.org/doc/html/rfc1961)GSSAPI authentication (not supported)|

## Logs
Socksprox writes logs to two files by default, `/var/log/access.log` and `/var/log/error.log`.

Access logs have the format `yyyy-mm-dd-hh:mm:ss-[IP]:port-->socksprox-->[IP]:port|Message`.  Access logs are generated by actions made by clients and the ensuing proxy connection.
